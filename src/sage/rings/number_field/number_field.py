...
class NumberField_generic(number_field_base.NumberField):
    ...
    def reduced_gram_matrix(self):
        ...
        EXAMPLES:
            ...
            x
            sage: F.<alpha> = NumberField(x^4+x^2+712312*x+131001238)
            sage: F.reduced_gram_matrix() # low order bits
...
class NumberField_absolute(NumberField_generic):
    ...
    def Minkowski_embedding(self, B=None):
        r"""
        ...
        EXAMPLES:
            ...
            sage: F.Minkowski_embedding() # low-order bits
    ...
    def places(self, all_complex=False):
        """
        ...
        ##
        all_intervals = [ x[0] for x in self.defining_polynomial().roots(CIF) ]

        ## first, set up the real places
        if all_complex:
            real_intervals = [ x for x in all_intervals if x.imag().is_zero() ]
        else:
            real_intervals = [ x[0] for x in self.defining_polynomial().roots(RIF) ]
    ...
    def real_places(self):
        ...
        """
        real_intervals = [ x[0] for x in self.defining_polynomial().roots(RIF) ]
        return [ self.hom([i.center()], check=False) for i in real_intervals ]

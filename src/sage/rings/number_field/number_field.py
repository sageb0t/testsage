...
class NumberField_generic(field.Field):
    ...
    def ideal(self, gens):
        ...
        overrides the \code{sage.ext.ring.Field} method to use the
        \code{sage.ext.ring.Ring} one instead, since we're not really
        concerned with ideals in a field but in its ring of integers.

        EXAMPLES:
            sage: K.<a> = NumberField(x^3-2)
            sage: K.ideal([a])
            Ideal (a) of Number Field in a with defining polynomial x^3 - 2
        """
        return sage.ext.ring.Ring.ideal(self, gens)

...
def Fricke_polynomial(l):
    r"""
    Fricke polynomials for ``l`` =2,3,5,7,13.
    ...
    The polynomial `P(t)` as en element of `\ZZ[t]`.
...
def Fricke_module(l):
    r"""
    Fricke functions for ``l`` =2,3,5,7,13.
...
def Psi(l, use_stored=True):
    r"""
    Generic kernel polynomials for genus one primes.
    ...
    All of this data is quickly computed from the Fricke modules,
    except that for `l=13` the factorization of the Generic Division
    Polynomial takes a long time (over 10 hours!), so the value have
    been precomputed and cached; by default the cached values are
    used, but the code here will recompute them when ``use_stored`` is
    ``False``, as in the doctests.
    ...
    .. note::
       ...
       This computation takes a negligible time for `l=2,3,5' and
       about 5s for `l=7`, but over 10 hours for `l=13`.  The reason
    ...
    TESTS::
        ...
        sage: assert Psi(7, use_stored=True) == Psi(7, use_stored=False) # long time (5s)

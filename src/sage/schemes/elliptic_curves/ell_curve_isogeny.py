...
# Definition of Fricke functions for l=3,5,7,13:

Qt = PolynomialRing(QQ,'t')
t = Qt.gen()
j3 = (t+3)**3*(t+27) / t
j5 = (t**2+10*t+5)**3 / t
j7 = (t**2+5*t+1)**3 * (t**2+13*t+49) / t
j13 = (t**2+5*t+13)*(t**4+7*t**3+20*t**2+19*t+1)**3 / t
Fricke_modules = dict([(3,j3),(5,j5),(7,j7),(13,j13)])

# Definition of generic division polynomial factors:
R = PolynomialRing(ZZ,'X,t')
X,t = R.gens()

# Since these take some time to compute (480s for l=7, ?? for l=13) we
# precompute them.  Here is how they may be computed:
#
#ZXT.<x,T> = PolynomialRing(ZZ,2)
### Next line takes 4s
#f = E_from_j(Fricke_modules[5](T)).division_polynomial(5,x).factor()[2][0]
#Psi5 = sum([X**i * f.coefficient({x:i})(X,t) for i in range(3)]).monic()
#
### Next line takes 482s
#f = E_from_j(Fricke_modules[7](T)).division_polynomial(7,x).factor()[1][0]
#Psi7 = sum([X**i * f.coefficient({x:i})(X,t) for i in range(4)]).monic()

Psi2 = X + (t + 16)/(t - 8)
Psi3 = X + (t**2 + 30*t + 81)/(t**2 + 18*t - 27)
Psi5 = X**2 + ((2*t**2 + 20*t + 10)/(t**2 + 4*t - 1))*X + (t**6 + 42*t**5 + 639*t**4 + 4300*t**3 + 12015*t**2 + 9450*t + 2225)/(t**6 + 30*t**5 + 315*t**4 + 1300*t**3 + 1575*t**2 - 978*t + 125)
...
def isogenies_prime_degree_genus_0(E, l=None):
    ...
    if l != None:
        fp = Fricke_modules[l]
        f = fp.numerator()
        psi = isogeny_kernel_polys[l]
        R = PolynomialRing(F,'t')
        t = R.gen()
        t_list = (R(f)-j*t).roots(multiplicities=False)
        t_list.sort()
        X = t
        kernels = [psi(X,t0) for t0 in t_list]
        c4, c6 = E.c_invariants()
        T = c4/(3*c6)
        kernels = [(1/T**ker.degree()) * ker(X*T) for ker in kernels]
...
def isogenies_7_1728(E):
    ...
    ts = (j7-1728).numerator().roots(F,multiplicities=False)
    if len(ts)==0:
        return []
    ts.sort()
    isogs = []

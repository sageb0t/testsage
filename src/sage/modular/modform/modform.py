...
class ModularFormsSpace(hecke.HeckeModule_generic):
    ...
    def __add__(self, right):
        ...
        return ModularFormsSubspace(self.ambient_space(), V)
    ...
    def __create_newspace(self, basis, level, t, is_cuspidal):
        V = self.vector_space().subspace(basis, check=False)
    ...
    def __subspace_from_subset_of_basis(self, x):
        V = self.vector_space()
        return V.subspace([V.gen(i) for i in x], check=False)
    ...
    def sturm_bound(self, M=None):
        ...
        Useful Remark:
            ...
            one sees by taking a power of $f$.  More precisely, if
            $f\con 0\pmod{p}$ for first $s$ coefficients, then $f^r
            = 0 \pmod{p}$ for first $s r$ coefficents.  Since the
            weight of $f^r$ is $r \text{weight}(f)$, it follows that
            if $s \geq $ the sturm bound for $\Gamma_0$ at weight(f),
            then $f^r$ has valuation large enough to be forced to be
            $0$ at $r\cdot$ weight(f) by Sturm bound (which is valid
            if we choose $r$ right).  Thus $f \con 0 \pmod{p}$.
            Conclusion: For $\Gamma_1$ with fixed character, the
            Sturm bound is \emph{exactly} the same as for $\Gamma_0$.
            A key point is that we are finding $\Z[\eps]$ generators
            for the Hecke algebra here, not $\Z$-generators.  So if
            one wants generators for the Hecke algebra over $\Z$,
            this bound is wrong.
    ...
    def cuspidal_subspace(self):
        if self.__is_cuspidal == True:
            return self
        if self.__cuspidal_subspace != None:
            return self.__cuspidal_subspace
        if self.is_ambient():
            # By definition the cuspidal subspace of the ambient space
            # is spanned by the first n standard basis vectors, where
            # n is the dimension of the cuspidal subspace.
            n = self.__ambient_cusp_dimension()
            W = self.__subspace_from_subset_of_basis(range(n))
            S = ModularForms(self, W)
            S.__is_cuspidal = True
            S.__is_eisenstein = (n==0)
            self.__cuspidal_subspace = S
            return S
        C = self.ambient_space().cuspidal_subspace()
        S = self.intersect(C)
    ...
    def decomposition(self):
        ...
        This function returns a list of subspaces $V(f_i,t)$
    ...
    def newspaces(self):
        r"""
        This function returns a list of subspaces $S(M,t)$ and
        ...
        the image under $f(q) \mapsto f(q^t)$ of the new subspace of
        cusp forms of level $M$, and similarly $E(M,t)$ is the image of
        Eisenstein series.

        Notes: (1) the subspaces $S(M,t)$ need not be stable under
        Hecke operators of index dividing $N/M$.  (2) Since self can
        be an arbitrary subspace, there's no guarantee any $S(M,t)$ or
        $E(M,t)$ is in self, so the return list could be empty.
        """
        V = self.embedded_subspace()
    ...
    def eisenstein_subspace(self):
        if self.__is_eisenstein == True:
            return self
        if self.__eisenstein_subspace != None:
            return self.__eisenstein_subspace
        if self.is_ambient():
            # By definition the eisenstein subspace of the ambient space
            # is spanned by the n+1 through n+d standard basis vectors, where
            # n is the dimension of the cuspidal subspace and d
            # is the dimension of the eisenstein subspace (i.e., the
            # number of eisenstein series).
            n = self.__ambient_cusp_dimension()
            d = self.__ambient_eis_dimension()
            W = self.__subspace_from_subset_of_basis(range(n,n+d))
            E = ModularForms(self, W)
            E.__is_eisenstein = True
            E.__is_cuspidal = (d==0)
            self.__eisenstein_subspace = E
            return E
        A = self.ambient_space().eisenstein_subspace()
        E = self.intersect(A)
    ...
    def embedded_subspace(self):
        if self.is_ambient():
            return self.vector_space()
        return self.__embedded_subspace
    ...
    def intersect(self, right):
        if self.ambient_space() != right.ambient_space():
            raise ArithmeticError, "Intersection of %s and %s not defined."%\
                                   (self, right)
        V = self.embedded_subspace().intersect(right.embedded_subspace())
...
class ModularForms(_uniqModularForms, ModularFormsSpace):
    ...
    def change_base(self, F):
        return ModularForms(self.weight(), self.group(), F)
    ...
    def qexp(self, vector, prec):
        ...
        """
        R = rings.PowerSeriesRing(self.base_field())
        return rings.q
        raise NotImplementedError

    ####################################################################
    # Computation of Special Subspaces
    ####################################################################
    ...
    def cuspidal_subspace(self):
        try:
            ...
            n = self.dim_cuspidal()
            V = self.vector_space()
            W = V.subspace([V.gen(i) for i in range(n)])
            self.__cuspidal_subspace = ModularFormsSubspace(self, W)
        return self.__cuspidal_subspace
    ...
    def eisenstein_subspace(self):
        try:
            ...
            d = self.dimension()
            V = self.vector_space()
            W = V.subspace([V.gen(i) for i in range(n, d)])
            self.__eisenstein_subspace = ModularFormsSubspace(self, W)
        return self.__eisenstein_subspace

    def new_subspace(self):
        try:
            return self.__new_subspace
        except AttributeError:
            ...
            V = self.vector_space()
            W = V.subspace([V.gen(i) for i in B])
            self.__new_subspace = ModularFormsSubspace(self, W)
        return self.__new_subspace
    ...
    def eisenstein_series(self):
        ...
        for i in range(len(params)):
            ...
            if chi.base_ring() != self.base_field():
                F = chi.base_ring()
                M = self.change_base(F)
                v = V.change_base(F).gen(c+i)
            else:
                M = self
                v = V.gen(c+i)
...
class ModularFormsWithCharacter(_uniqModularForms_chars, ModularForms):
    ...
    def __init__(self, character, weight, base_field=None):
        ...
        if character.base_ring() != base_field:
            character = character.change_base(base_field)
...
class ModularFormsSubspace(ModularFormsSpace):
    """
    ...
    def __init__(self, ambient_space, vector_space):
        """
            ...
            subspace -- a vector subspace of the underlying vector space of the ambient space.
        """
        self.__ambient_space = ambient_space
        self.__vector_space = vector_space
        A = ambient_space
        ModularFormsSpace.__init__(self, A.weight(), A.group(), A.character(), A.base_field())

    def __repr__(self):
        return "ModularFormsSubspace(%s,dim=%s)"%(self.ambient_space(), self.dimension())
        #return "Subspace of dimension %s of modular forms of weight %s on %s with character %s."%(
...
class ModularFormElement:
    ...
    def __getslice__(self, i, j):
        return self.cooefficients(i,j+1)
...
class EisensteinSeries(ModularFormElement):
    ...
    def __defining_parameters(self):
        try:
            ...
            chi = chi.change_base_ring(K)
            psi = psi.change_base_ring(K)
            t = self.__t
            L = chi.conductor()
            M = psi.conductor()
            if L == 1:

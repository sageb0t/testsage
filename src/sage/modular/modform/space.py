...
`\mathbb{Q}` (if the characteristic is `0`).
...
class ModularFormsSpace(hecke.HeckeModule_generic):
    ...
    def q_integral_basis(self, prec=None):
        r"""
        Return a `\mathbb{Z}`-reduced echelon basis of
        `q`-expansions for self.

        The `q`-expansions are power series with coefficients in
        `\mathbb{Z}`; they are *not* actual modular forms.

        The base ring of self must be `\mathbb{Q}`. The number of
    ...
    def sturm_bound(self, M=None):
        ...
        much better bounds over `\mathbb{Q}`, or for comparing two
        eigenforms.
        """
        if M is not None:
            raise NotImplementedError

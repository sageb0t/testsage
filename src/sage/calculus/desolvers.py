...
def desolve(de,vars):
    ...
    EXAMPLES:
        sage: desolve("x**2*diff(y,x) + 3*y*x = sin(x)/x",["x","y"])
        y=(%c-cos(x))/x^3
...
def desolve_laplace(de,vars,ics=None):
    ...
    EXAMPLES:
        sage: desolve_laplace("diff(f(x),x,2)=2*diff(f(x),x)-f(x)",["x","f"])
        f(x)=x*%e^x*(?%at('diff(f(x),x,1),x=0))-f(0)*x*%e^x+f(0)*%e^x
        sage: desolve_laplace("diff(f(x),x,2)=2*diff(f(x),x)-f(x)",["x","f"],[0,1,2])
        f(x)=x*%e^x+%e^x
...
def desolve_system(des,vars,ics=None):
	...
	sage: des=["'diff(x(t),t)=-4*y(t)","'diff(y(t),t)=-x(t)"]
    	sage: vars = ["t","x","y"]
    	sage: desolve_system(des,vars,[0,270,90])

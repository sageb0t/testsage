...
# 3dproj.py
#
# Copyright (C) 2005 Cambridge Broadband Ltd
# Author: John Porter <jdp@cambridgebroadband.com>
# Created: 25 Aug 2005
#
# Desc:
#
...
def line2d(p0,p1):
    ...
    #y/(y0-y1) - y1/(y0-y1) = x/(x0-x1) - x1/(x0-x1)
    #y/(y0-y1) - x/(x0-x1) + x1/(x0-x1) - y1/(y0-y1) = 0
    # x + x1  = 0
    x0,y0 = p0[:2]
    x1,y1 = p1[:2]
    #
...
def test_proj():
    ...
    for x,y,t in zip(txs,tys,ts):
        pylab.text(x,y,t)
    #
    #pylab.xlim(min(txs),max(txs))
    #pylab.ylim(min(tys),max(tys))
    pylab.xlim(-0.2,0.2)
    pylab.ylim(-0.2,0.2)
    #
    pylab.show()

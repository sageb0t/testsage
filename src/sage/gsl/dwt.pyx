...
def WaveletTransform(size_t n, wavelet_type,size_t wavelet_k):
  ...
  can perform a discrete wavelet transform. n is requiried to be a power of 2.
  T  the data in the GSLDoubleArray must be real. wavelet_type is the name of
  the type of wavelet, valid choices are: 'daubechies','daubechies_centered',
  'haar','haar_centered','bspline', and 'bspline_centered'. For daubechies
  wavelets, wavelet_k specifies a daubechie wavelet with k/2 vanishing moments.
  k = 4,6,...,20 for k even are the only ones implemented. For Haar wavelets
  wavelet_k must be 2. For bspline wavelet_k = 103,105,202,204,206,208,301,305,
  307,309 will give biorthogonal B-spline wavelets of order (i,j) where
  wavelet_k=100*i+j. The wavelet transfrom uses J=log_2(n) levels.
  The output will be an array of the form (s_{-1,0},d_{0,0},d_{1,0},d_{1,1},
  d_{2,0}...,d_{J-1,2^{J-1}-1}) for d_{j,k} the detail coefficients of level j.
...
This example gives a simple example of wavelet compression.
        ...
        sage: for i in range(2048):
        ...     a[i]=float(sin((i*5/2048)**2))
        sage.: show(plot(a))
        sage: a.forward_transform()
        sage: for i in range(1800):
        ...     a[2048-i-1]=0

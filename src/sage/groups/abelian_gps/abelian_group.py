...
In this situation, `k=\ell` and `\phi:  \mathbb{Z}^\ell \rightarrow A` is the map
`\phi(x_1,...,x_\ell) = a_1^{x_1}...a_\ell^{x_\ell}`,
for `(x_1,...,x_\ell)\in \mathbb{Z}^\ell`. The matrix of relations
`M:\mathbb{Z}^k \rightarrow \mathbb{Z}^\ell` is the matrix
whose rows generate the kernel of `\phi` as a `\mathbb{Z}`-module.
...
The kernel of the map `\phi_B:  \mathbb{Z}^m \rightarrow B` defined by
`\phi_B(y_1,...,y_m) = b_1^{y_1}...b_m^{y_m}`,
for `(y_1,...,y_m)\in \mathbb{Z}^m`, is the kernel of the matrix
...
regarded as a map
`\mathbb{Z}^m\rightarrow (\mathbb{Z}/p_1^{c_1}\mathbb{Z})\times ...\times (\mathbb{Z}/p_\ell^{c_\ell}\mathbb{Z})`.
In particular, `B\cong \mathbb{Z}^m/ker(F)`. If `B=A` then the

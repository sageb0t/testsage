dnl *********************************************************************
dnl  Intel64 mpn_mul_1 -- Multiply a limb vector with a limb and
dnl  store the result.
dnl
...
PROLOGUE(mpn_mul_1)
	...
	cmp	$THRESHOLD,%rdx
	jge	L_mpn_mul_1_main_loop_prep
	movq	%rdx, %r11
	leaq	(%rsi,%rdx,8), %rsi
	leaq	(%rdi,%rdx,8), %rdi
	negq	%r11
	xorq	%r8, %r8
	xorq	%r10, %r10
	jmp	L_mpn_mul_1_small_loop
	ALIGN(16)
L_mpn_mul_1_small_loop:
	movq	(%rsi,%r11,8), %rax
	mulq	%rcx
	addq	%r8, %rax
	movq	%r10, %r8
	...
	adcq	%rdx, %r8
	incq	%r11
	jne	L_mpn_mul_1_small_loop

	movq	%r8, %rax
	ret

L_mpn_mul_1_main_loop_prep:
	pushq	%r15
	pushq	%r14
	pushq	%r13
	pushq	%r12
	...
	movq	$0, A_y
	andq	$1, %rdx
	jnz	L_mpn_mul_1_odd_n

					C Case n is even
	movq	ADDR(s1p,index,0), %rax
	mulq	s2limb
	movq	%rax, ADDR(rp,index,0)
	addq	%rdx, A_x
	addq	$1, index
					C At this point
					C  temp = n (even)
...
L_mpn_mul_1_odd_n:
	...
	jz	L_mpn_mul_1_main_loop
	shrq	$1, temp
L_mpn_mul_1_main_loop_spin_up:
	...
	addq	$2, index
	subq	$1, temp
	jnz	L_mpn_mul_1_main_loop_spin_up

	jmp	L_mpn_mul_1_main_loop
	ALIGN(16)
L_mpn_mul_1_main_loop:
